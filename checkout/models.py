...
class Order(models.Model):
    ...
    def update_total(self):
        ...
        default_shipping_cost = (
            50  # Default value if shipping settings are not available
        )

        # Fallback to default if settings are missing or improperly configured
        shipping_cost = getattr(
            shipping_settings, "standard_shipping_cost", default_shipping_cost
        )
...
class OrderLineItem(models.Model):
    ...
    product = models.ForeignKey(
        Microcontroller, null=False, blank=False, on_delete=models.CASCADE
    )
